...
#include "esphome/components/text_sensor/text_sensor.h" // Thêm thư viện Text Sensor
...
class IECReaderComponent : public Component {
 public:
  ...
  // Khai báo các Text Sensor
  text_sensor::TextSensor *meter_id_sensor = new text_sensor::TextSensor();
  text_sensor::TextSensor *meter_time_sensor = new text_sensor::TextSensor(); // Giờ
  text_sensor::TextSensor *meter_date_sensor = new text_sensor::TextSensor(); // Ngày
  ...
  // --- LOGIC PHÂN TÍCH VÀ GIAO TIẾP (GIỮ NGUYÊN) ---
  ...
  void parse_and_publish(const std::string& data_block) {
    // Trích xuất và chuyển đổi sang float
    ...
    // --- TRÍCH XUẤT TEXT SENSOR ---
    std::string meter_id = extract_obis_value(data_block, "0.0.C.1.0"); // Meter ID
    ...
    if (!meter_time.empty()) meter_time_sensor->publish_state(meter_time);
    if (!meter_date.empty()) meter_date_sensor->publish_state(meter_date);
    
    ESP_LOGI(TAG, "Data published: V=%.2f, Time=%s, Date=%s", voltage, meter_time.c_str(), meter_date.c_str());
  }


  // (Các hàm giao tiếp Serial send_and_read và read_data_block giữ nguyên)
